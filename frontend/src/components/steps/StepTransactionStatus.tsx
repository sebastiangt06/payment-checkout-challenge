// src/components/steps/StepTransactionStatus.tsx
...
export const StepTransactionStatus: React.FC = () => {
  ...
  const handleReturnToShop = () => {
    // 1. Limpia el estado del proceso de compra
    dispatch(resetFlow());
    // 2. Consulta PostgreSQL/RDS para obtener el stock actualizado
    dispatch(fetchProducts());
  };

  const productTotal = selectedProduct ? selectedProduct.price * quantity : 0;
  const grandTotal = productTotal + 2500 + 10000; // Producto + Base Fee ($2.500) + Delivery Fee ($10.000)
  ...
  return (
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-[375px] sm:max-w-md bg-white rounded-2xl border border-slate-100 shadow-2xl p-6 text-center animate-slide-up">
        ...
        {/* Botón para retornar a la tienda */}
        <button
          type="button"
